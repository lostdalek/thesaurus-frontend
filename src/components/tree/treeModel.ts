 ...
 * https://github.com/wangzuo/tree.js
 *
 */
// import * as Tree from 'js-tree';
interface ICollapsableTree {
    isNodeCollapsed?: boolean;
    renderNode?: Function;
    changeNodeCollapsed?: any;

    children: ICollapsableTree[];
}
interface ITreeIndex {
    id: number;
    node: ICollapsableTree;
    parent?: ICollapsableTree;
}

class TreeModel implements ICollapsableTree{
    public isNodeCollapsed;
    public changeNodeCollapsed;
    public children;
    private indexes;
    private cnt;
    private obj;
    constructor(obj: ICollapsableTree) {
        this.cnt = 1;
    ...
    build(obj: ICollapsableTree): ITreeIndex {
        var indexes = this.indexes;
        var startId = this.cnt;
        var self = this;

        var index: ITreeIndex = {id: startId, node: obj};
        indexes[this.cnt+''] = index;
        this.cnt++;



        let walk = function(objs: ICollapsableTree[], parent) {
            var children = [];
            objs.forEach(function(obj: ICollapsableTree, i) {
                var index: ITreeIndex = {
                    id: self.cnt,
                    node: obj
                };
    ...
    /**
     * following methods from https://github.com/pqx/react-ui-tree
     * The MIT License (MIT)
    ...
    updateNodesPosition()
    {
        ...
        let walk = function(children, parent, left, collapsed) {
            console.log('walking through', children, parent, left, collapsed)
            var height = 1;
            children.forEach((id) => {
